...
def run():
    ...
    def mod_logs_list():
        ...
        FROM LOGS 
        OUTER JOIN ASSETS ON ASSETS_ID = LOGS_ASSETS_ID AND ASSETS
        OUTER JOIN LIST_ITEMS ACTION ON ACTION.LIST_ITEMS_ID = LOGS_ACTION_ID AND ACTIONS.LISTS_ID = (SELECT LISTS_ID FROM LISTS WHERE LIST_NAME='LOG ACTIONS')
        OUTER JOIN LIST_ITEMS NODES ON NODE.LIST_ITEMS_ID = LOGS_NODE_ID AND NODE.LISTS_ID = (SELECT LISTS_ID FROM LISTS WHERE LIST_NAME='LOG NODES')
        OUTER JOIN PEOPLE_COLUMN_VALUES PFN ON PFN.PEOPLE_COLUMN_VALUES_PEOPLE_ID = LOGS_ACTION_PEOPLE_ID AND PFN.PEOPLE_COLUMN_VALUES_PEOPLE_COLUMNS_ID = (SELECT PEOPLE_COLUMNS_ID FROM PEOPLE_COLUMNS WHERE PEOPLE_COLUMNS_NAME='FIRST NAME')
        OUTER JOIN PEOPLE_COLUMN_VALUES PLN ON PLN.PEOPLE_COLUMN_VALUES_PEOPLE_ID = LOGS_ACTION_PEOPLE_ID AND PLN.PEOPLE_COLUMN_VALUES_PEOPLE_COLUMNS_ID = (SELECT PEOPLE_COLUMNS_ID FROM PEOPLE_COLUMNS WHERE PEOPLE_COLUMNS_NAME='LAST NAME')
        """
        if query:
            query = "%" + query + "%"
